...
public abstract class WebUtils {
	...
	public static String doPost(String url, String ctype, byte[] content,int connectTimeout,int readTimeout) throws IOException {
		...
		try {
			try{
				...
				System.out.print("Error:" + url + ":" + content);
				throw e;
			}
			try{
	...
	public static String doPost(String url, Map<String, String> params,
		...
		try {
			try{
				...
				Map<String, String> map=getParamsFromUrl(url);
				System.out.print("Error:" + url + ":" + params);
				throw e;
			}
	...
	public static Response httpRequest(String url, PostParameter[] postParams, String httpMethod) throws Exception {
		int retriedCount;
		int retry = 3 ;
		Response res = null;
...
//					setHeaders(url, postParams, con, httpMethod);
					...
					if (responseCode != 200) {
						if (responseCode < 500 || retriedCount == 3) {
							throw new Exception(responseCode + "\n" + res.asString() + "; response code = " + responseCode);
						}
				...
				if (retriedCount == 3) {
					throw new Exception(ioe.getMessage() + "\\" + ioe +	"; response code = " + responseCode);
				}else{
					ioe.printStackTrace();
				}
			}
...
//		conn.setRequestProperty("Accept", "text/xml,text/javascript,text/html");
		conn.setRequestProperty("User-Agent", "My Client");
	...
	private static URL buildGetUrl(String strUrl, String query) throws IOException {
		...
		if (FileUtil.isEmptyString(query)) {
			return url;
		}

		if (FileUtil.isEmptyString(url.getQuery())) {
			if (strUrl.endsWith("?")) {
				strUrl = strUrl + query;
			} else {
				strUrl = strUrl + "?" + query;
	...
	public static String buildQuery(Map<String, String> params, String charset) throws IOException {
		...
		for (Entry<String, String> entry : entries) {
			...
			// 忽略参数名或参数值为空的参数
			if (FileUtil.areNotEmptyString(name, value)) {
				if (hasParam) {
					query.append("&");
				} else {
					hasParam = true;
	...
	private static String getResponseCharset(String ctype) {
		...
		if (!FileUtil.isEmptyString(ctype)) {
			...
			for (String param : params) {
				...
				if (param.startsWith("charset")) {
					...
					if (pair.length == 2) {
						if (!FileUtil.isEmptyString(pair[1])) {
							charset = pair[1].trim();
						}
					}
					break;
	...
	public static String decode(String value, String charset) {
		String result = null;
		if (!FileUtil.isEmptyString(value)) {
			try {
